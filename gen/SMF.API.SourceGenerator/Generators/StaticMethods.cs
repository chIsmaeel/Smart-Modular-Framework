    ...
    internal static object ModifiedEntityImplicitOperatorProperties(ModelCT s, string objName, bool allowComputedValues = true, bool addCreatedOn = false, bool addLastModifiedOn = false)
    {
        ...
        while (tempModelCT is not null)
        {
            foreach (var property in tempModelCT.Properties!)
            {
                ...
                {
                    sb.AppendLine($"resultObj.{identifer}_ID = {objName}.{identifer}_ID;");
                    continue;
                }
                if (property.Type is not null && property.Type!.StartsWith("SMFields.M2") && property.RelationshipWith is not null)
                {
                    sb.AppendLine($"if({objName}.{identifer.Pluralize()}_IDs?.Count > 0){{");
                    sb.AppendLine($"resultObj.{identifer}_IDs = new ();");
                    sb.AppendLine($"resultObj.{identifer}_IDs.AddRange({objName}.{identifer}_IDs);");
                    sb.AppendLine($"}}");
                    continue;
                }
...
");
 ...
 ");
    ...
    internal static void AddProperties(ModelCT tempModelCT, StringBuilder messages)
    {
        foreach (var property in tempModelCT.Properties!)
        {
            var identifer = property!.IdentifierName;
            ...
            //if (!property.Type.StartsWith("SMFields"))
            //    continue;
            var type = ModelPropertyTypes.GetPropertyType(property!);
            if (type is not null && property.HasRelation is not null)
                messages.AppendLine("[System.Text.Json.Serialization.JsonIgnore]");

            if (property.HasRelation is not null)
            {
                if (property.Type.StartsWith("System.Collections.Generic.ICollection"))
                    messages.AppendLine(GetPropertyTemplate($"System.Collections.Generic.ICollection<{(property.HasRelation.HasRelation.ClassType as ModelCT)!.NewContainingNamespace}.{(property.HasRelation.HasRelation.ClassType as ModelCT)!.IdentifierNameWithoutPostFix}Dto>", identifer));

                else if (property.Type.StartsWith("System.Collections.Generic.List"))
                    messages.AppendLine(GetPropertyTemplate($"System.Collections.Generic.List<{(property.HasRelation.HasRelation.ClassType as ModelCT)!.NewContainingNamespace}.{(property.HasRelation.HasRelation.ClassType as ModelCT)!.IdentifierNameWithoutPostFix}Dto>", identifer.Pluralize()));
                else
                    messages.AppendLine(GetPropertyTemplate($"{(property.HasRelation.HasRelation.ClassType as ModelCT)!.NewContainingNamespace}.{(property.HasRelation.HasRelation.ClassType as ModelCT)!.IdentifierNameWithoutPostFix}Dto", identifer));
            ...
            if ((bool)type?.StartsWith("System.Collections.Generic")!)
                identifer = identifer.Pluralize();
            messages.AppendLine(GetPropertyTemplate(type, identifer));
        }
    }
    ...
    /// <param name="messages">The messages.</param>
    /// <param name="i">The i.</param>
    internal static void CreateCommandProperties(ModelCT tempModelCT, StringBuilder messages)
    {
        foreach (var property in tempModelCT.Properties!)
        {
            if (property!.IdentifierName.EndsWith("_FK"))
            ...
            {
                messages.AppendLine(GetPropertyTemplate("int", identifer + "_ID"));
                continue;
            }

            if (property.Type is "SMFields.M2O" or "SMFields.M2M")
            {
                messages.AppendLine(GetPropertyTemplate("System.Collections.Generic.List<int>", identifer + "_IDs"));
                continue;
            }
            var type = ModelPropertyTypes.GetPropertyType(property!);
            if (type is null) continue;
    ...
    /// <param name="messages">The messages.</param>
    /// <param name="i">The i.</param>
    internal static void UpdateCommandProperties(ModelCT tempModelCT, StringBuilder messages)
    {
        foreach (var property in tempModelCT.Properties!)
        {
            if (property!.IdentifierName.EndsWith("_FK"))
    ...
    public static string GetPropertyTemplate(string type, string name)
    {
